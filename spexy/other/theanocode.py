...
class TheanoPrinter(Printer):
    ...
    def _print_Sum(self, expr, **kwargs):
        e, (k, k0, kN) = expr.args
        e = self._print(e, )
        #rslt, _ = theano.scan(self._print(e, **kwargs),
        #                      sequences=self._print(k, **kwargs))
        raise NotImplementedError
...
def theano_function(inputs, outputs, dtypes={}, cache=None, **kwargs):
    ...
    for tin in tinputs:
        for i, tout in enumerate(toutputs):
            toutputs[i]+=0*tin

    func = theano.function(tinputs, toutputs, **theano_kwargs)
